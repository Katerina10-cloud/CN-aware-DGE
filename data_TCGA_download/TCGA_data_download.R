...
query_TCGA_cnv <- GDCquery(project = 'TCGA-LIHC',
...
query_TCGA_rna_tumor <- GDCquery(project = 'TCGA-LIHC',
...
query_TCGA_rna_normal <- GDCquery(project = 'TCGA-LIHC',
                           data.category = 'Transcriptome Profiling',
                           experimental.strategy = 'RNA-Seq',
                           workflow.type = 'STAR - Counts',
                           data.type = "Gene Expression Quantification",
...
### Prepare data ###
lihc_cnv_seg <- GDCprepare(query_TCGA_cnv)

lihc_cnv_tumor <- GDCprepare(query_TCGA_cnv, summarizedExperiment = TRUE)
...
lihc_cnv_tum <- lihc_cnv_tum[!duplicated(lihc_cnv_tum$GeneID), ] %>% remove_rownames %>% column_to_rownames(var="GeneID") %>%
  na.omit()

lihc_rna_normal <- GDCprepare(query_TCGA_rna_normal, summarizedExperiment = TRUE)
...
lihc_rna_norm <- lihc_rna_norm[!duplicated(lihc_rna_norm$GeneID), ] %>% remove_rownames %>% column_to_rownames(var="GeneID") %>%
  na.omit()
