...
query_TCGA_rna_tumor <- TCGAbiolinks::GDCquery(project = 'TCGA-LUSC',
...
query_TCGA_rna_normal <- TCGAbiolinks::GDCquery(project = 'TCGA-LUSC',
                           data.category = 'Transcriptome Profiling',
                           experimental.strategy = 'RNA-Seq',
                           workflow.type = 'STAR - Counts',
                           data.type = "Gene Expression Quantification",
...
# Data preprocessing #
rna_tum <- rna_tum[,(colnames(rna_tum) %in% colnames(rna_norm))] 
rna_normal<- rna_normal[,(colnames(rna_norm) %in% colnames(cnv_tum))]
...
saveRDS(cnv_tum, file = "lung/LUSC/cnv_tumor.RDS")
saveRDS(rna_norm, file = "lung/LUSC/rna_normal.RDS")
saveRDS(rna_tum, file = "lung/LUSC/rna_tumor.RDS")
